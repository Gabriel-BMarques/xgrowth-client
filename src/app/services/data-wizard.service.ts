...
export class DataWizardService {
  ...
  prepareDataToSubmit() {
    ...
    if (this.step1Form) {
      ...
      this.entity.UploadedFiles = [];
      Object.entries(this.step1Form.controls.uploadedFiles.value).forEach(([key, url]) => {
        this.entity.UploadedFiles[key] = {};
        this.entity.UploadedFiles[key].url = url;
      });
      // this.entity.UploadedFiles = this.step1Form.controls.uploadedFiles.value; (ON HOLD)
    }
    if (this.step2Form) {
